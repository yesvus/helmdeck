...
export default function OrdersPage() {
  ...
  return <div className="space-y-6"><header><p className="text-sm text-zinc-500">Review payments, fulfillment, and customer details.</p></header>
    <section className="grid gap-4 sm:grid-cols-3">{[["Today", "12", "$1,684"], ["Awaiting fulfillment", "8", "Ready to ship"], ["Refund requests", "2", "Needs review"]].map(([label, value, sub]) => <article key={label} className="rounded-xl border border-zinc-200 bg-admin-surface p-4"><p className="text-sm text-zinc-500">{label}</p><p className="mt-2 text-xl font-bold">{value}</p><p className="mt-1 text-xs text-zinc-500">{sub}</p></article>)}</section>
    <section className="rounded-xl border border-zinc-200 bg-admin-surface p-5"><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="font-semibold">All orders</h2><p className="mt-1 text-sm text-zinc-500">{orders.length} matching orders</p></div><button onClick={() => setNotice("Order report prepared for this demo.")} className="rounded-lg border border-zinc-300 px-3 py-2 text-sm font-medium hover:bg-zinc-50 focus-visible:outline-2">Export report</button></div>
      <div className="mt-4 flex flex-wrap gap-3"><label className="sr-only" htmlFor="order-search">Search orders</label><input id="order-search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search order or customer" className="min-w-56 rounded-lg border border-zinc-300 px-3 py-2 text-sm"/><label className="sr-only" htmlFor="order-status">Filter by status</label><select id="order-status" value={status} onChange={(event) => setStatus(event.target.value)} className="rounded-lg border border-zinc-300 bg-admin-surface px-3 py-2 text-sm"><option>All orders</option>{[...new Set(demoOrders.map((item) => item.status))].map((item) => <option key={item}>{item}</option>)}</select></div>
